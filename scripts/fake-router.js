...
export function createRouter(loadPageModule, fallbackComponent) {
  ...
  if (inBrowser) {
    ...
    window.addEventListener("popstate", async (e) => {
      ...
      let href = normalizeHref(location.href);
      // Handle /docs prefix: strip it for page loading if not on docs subdomain
      const isDocsHost =
        location.hostname.startsWith("docs.") ||
        location.hostname.startsWith("localhost") ||
        location.hostname.startsWith("127.0.0.1");
      if (!isDocsHost && href.startsWith("/docs/")) {
        href = href.replace("/docs/", "/");
      } else if (!isDocsHost && href === "/docs") {
        href = "/";
      }
