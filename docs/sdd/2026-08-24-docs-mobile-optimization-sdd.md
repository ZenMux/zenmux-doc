...
At mobile widths, the 60px Header uses VitePress's native Header hamburger and navigation screen to expose the existing locale-aware website navigation (`theme.nav`). The breadcrumb category icon proxies the hidden VitePress local-navigation menu button, so it opens the Docs sidebar while preserving VitePress's native sidebar state, focus restoration, Escape handling, route closing, and scroll locking. The sidebar renders `DocTabs` in `sidebar` mode before the configured locale-aware navigation tree. Markdown H1 closing tokens append a mobile-only `Copy` instance so the reading order is breadcrumb, title, Copy Page, then content.

The implementation follows Figma node `12160:2162`: the Header uses a 20px website-menu icon, an 88.354px by 20px logo, 36px appearance/language controls, and a 40px account treatment. The breadcrumb uses a separate 20px Docs-menu icon with a 40px hit area. The article uses a `#fafafa` light surface with 20px top and 16px horizontal padding. Mobile typography is 14px for breadcrumbs, 28px bold for H1, 20px bold for H2, and 16px regular for body copy, with the 8px and 28px vertical gaps shown in the design.

The opened Header navigation follows the `Menu_mobile` variants under Figma node `12176:2366`. It becomes a full-width screen below the 60px Header, moves the same 20px hamburger into a right-aligned close control, and hides the other Header actions while open. Menu content uses 12px outer padding, 14px/700 top-level labels, 57px collapsed rows with inset 0.5px dividers, 14px chevrons, and 15px/400 child rows with 34px height and 6px radius. VitePress continues to own each group's expansion state and page scroll lock.
