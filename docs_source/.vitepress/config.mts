...
export default defineConfig({
  ...
  head: [
    ...
    [
      ...
      // 历史链接/书签/AI 输出多为旧格式（无 /docs，如 /zh/best-practices/x），需补 /docs；
      // 但若 pathname 已含 /docs（base=/docs/ 原生路径）则不能再补，否则双前缀。
      if (window.location.hostname === 'docs.zenmux.ai') {
        var docsPath = window.location.pathname;
        if (docsPath !== '/docs' && docsPath.indexOf('/docs/') !== 0) {
          docsPath = '/docs' + docsPath;
        }
        window.location.replace('https://zenmux.ai' + docsPath + window.location.search + window.location.hash);
      }
      `,
    ],
    [
