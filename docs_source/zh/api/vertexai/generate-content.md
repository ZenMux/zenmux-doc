...
#### role `string` <span style="color: #666; font-weight: 400">可选</span>
...
- text `string`<span style="color: #666; font-weight: 400">可选</span>

  文本提示或代码段。

- inlineData `Blob`<span style="color: #666; font-weight: 400">可选</span>

  原始字节中的内嵌数据。
  - mimeType `string` <span style="color: #666; font-weight: 400">可选</span>
  ...
  - data `bytes` <span style="color: #666; font-weight: 400">可选</span>

    原始字节中的内嵌数据。

- fileData `FileData`<span style="color: #666; font-weight: 400">可选</span>

  存储在文件中的数据。
  - mimeType `string` <span style="color: #666; font-weight: 400">可选</span>
  - fileUri `string` <span style="color: #666; font-weight: 400">可选</span>

    要包含在提示中的文件的 URI 或网址。

- functionCall `FunctionCall`<span style="color: #666; font-weight: 400">可选</span>
  ...
  - args `Record<string,any>` <span style="color: #666; font-weight: 400">可选</span>

    以 JSON 对象格式表示的函数参数和值。

- functionResponse `FunctionResponse`<span style="color: #666; font-weight: 400">可选</span>
  ...
  - response `Record<string,any>` <span style="color: #666; font-weight: 400">可选</span>

    以 JSON 对象格式表示的函数响应。

- videoMetadata `VideoMetadata` <span style="color: #666; font-weight: 400">可选</span>

  对于视频输入，为视频的开始和结束偏移量（采用时长格式），以及视频的帧速率。
  - startOffset `number` <span style="color: #666; font-weight: 400">可选</span>

    视频开始偏移量（时长格式）。

  - endOffset `number` <span style="color: #666; font-weight: 400">可选</span>

    视频结束偏移量（时长格式）。

  - fps `number` <span style="color: #666; font-weight: 400">可选</span>

    视频帧速率。

- mediaResolution `enum`<span style="color: #666; font-weight: 400">可选</span>
...
### cachedContent `string` <span style="color: #666; font-weight: 400">可选</span>

缓存内容资源名（作为上下文使用）：

- `projects/{project}/locations/{location}/cachedContents/{cachedContent}`

### tools `array<Tool>` <span style="color: #666; font-weight: 400">可选</span>

工具列表（如函数调用、检索、搜索、代码执行等）。

### toolConfig `ToolConfig` <span style="color: #666; font-weight: 400">可选</span>

工具配置（对本次请求里所有 tools 共享）。

<!-- ### labels `Record<string,string>` <span style="color: #666; font-weight: 400">可选</span>

用户自定义标签（用于计费与报表等）。 -->

### safetySettings `array<SafetySetting>` <span style="color: #666; font-weight: 400">可选</span>
...
#### method `string` <span style="color: #666; font-weight: 400">可选</span>
...
### generationConfig `GenerationConfig` <span style="color: #666; font-weight: 400">可选</span>
...
#### topP `number` <span style="color: #666; font-weight: 400">可选</span>
...
#### topK `integer` <span style="color: #666; font-weight: 400">可选</span>

Top-K 采样阈值：模型仅在概率最高的 `topK` 个 token 中采样。例如 `topK=40` 表示每步只从概率最高的 40 个 token 中选择下一个 token。值越小越保守，值越大越发散。不同模型默认值可能不同。

#### candidateCount `integer` <span style="color: #666; font-weight: 400">可选</span>
...
#### maxOutputTokens `integer` <span style="color: #666; font-weight: 400">可选</span>

最大输出 token 数，用于限制回答长度；token 粗略可理解为约 4 个字符（英文场景）。值越小输出越短，值越大允许更长输出。

#### stopSequences `array<string>` <span style="color: #666; font-weight: 400">可选</span>

停止序列列表：当模型输出中遇到任意 stop 序列时立即停止，并在首次出现位置截断；**大小写敏感**。列表最多 **5** 个元素。

#### presencePenalty `number` <span style="color: #666; font-weight: 400">可选</span>

出现惩罚：对已经在"已生成文本"中出现过的 token 施加惩罚，从而提高生成新内容/多样性的概率。取值范围 `-2.0 ~ <2.0`。

#### frequencyPenalty `number` <span style="color: #666; font-weight: 400">可选</span>
...
#### responseMimeType `string` <span style="color: #666; font-weight: 400">可选</span>
...
#### responseSchema `Schema` <span style="color: #666; font-weight: 400">可选</span>
...
- `enum` `array<string>` <span style="color: #666; font-weight: 400">可选</span>

  枚举值列表；模型只能从中选择一个输出。通常配合 `type: STRING` 使用。

- `items` `Schema` <span style="color: #666; font-weight: 400">可选</span>

  当 `type` 为 `ARRAY` 时，指定数组元素的 schema。

- `properties` `map<string, Schema>` <span style="color: #666; font-weight: 400">可选</span>

  当 `type` 为 `OBJECT` 时，定义对象中各属性及其 schema。

- `required` `array<string>` <span style="color: #666; font-weight: 400">可选</span>

  对象中必填属性列表。

- `propertyOrdering` `array<string>` <span style="color: #666; font-weight: 400">可选</span>

  指定对象属性的输出顺序。

- `nullable` `boolean` <span style="color: #666; font-weight: 400">可选</span>

  是否允许值为 `null`。

- `minimum` `number` <span style="color: #666; font-weight: 400">可选</span>

  当 `type` 为 `INTEGER` 或 `NUMBER` 时，指定允许的最小值。

- `maximum` `number` <span style="color: #666; font-weight: 400">可选</span>

  当 `type` 为 `INTEGER` 或 `NUMBER` 时，指定允许的最大值。

- `minItems` `integer` <span style="color: #666; font-weight: 400">可选</span>

  当 `type` 为 `ARRAY` 时，指定数组最少元素数。

- `maxItems` `integer` <span style="color: #666; font-weight: 400">可选</span>

  当 `type` 为 `ARRAY` 时，指定数组最多元素数。

- `minLength` `integer` <span style="color: #666; font-weight: 400">可选</span>

  当 `type` 为 `STRING` 时，指定字符串最小长度。

- `maxLength` `integer` <span style="color: #666; font-weight: 400">可选</span>

  当 `type` 为 `STRING` 时，指定字符串最大长度。

- `pattern` `string` <span style="color: #666; font-weight: 400">可选</span>
...
- `anyOf` `array<Schema>` <span style="color: #666; font-weight: 400">可选</span>
...
#### responseJsonSchema `object` <span style="color: #666; font-weight: 400">可选</span>
...
#### responseLogprobs `boolean` <span style="color: #666; font-weight: 400">可选</span>

是否返回输出 token 的 log probability（对数概率）。设为 `true` 时，在响应的 `logprobsResult` 中会包含每个生成 token 的对数概率信息。**使用 `logprobs` 字段前，必须先启用此参数**。

#### logprobs `integer` <span style="color: #666; font-weight: 400">可选</span>
...
#### responseModalities `array<enum>` <span style="color: #666; font-weight: 400">可选</span>
...
#### audioTimestamp `boolean` <span style="color: #666; font-weight: 400">可选</span>

音频时间戳理解：用于**纯音频文件**的时间戳理解能力（preview）。仅部分模型支持（例如部分 Gemini Flash 系列）。

#### thinkingConfig `ThinkingConfig` <span style="color: #666; font-weight: 400">可选</span>

Gemini 2.5 及更高版本的 "thinking"（内部推理）配置。对不支持 thinking 的模型设置此字段会返回错误。

- `thinkingBudget` `integer` <span style="color: #666; font-weight: 400">可选</span>
...
- `thinkingLevel` `enum (ThinkingLevel)` <span style="color: #666; font-weight: 400">可选</span>
...
- `includeThoughts` `boolean` <span style="color: #666; font-weight: 400">可选</span>

  是否在响应中返回思考过程摘要。设为 `true` 时，在可用的情况下会返回 thought 摘要。属于 best-effort 能力，即使设置了也不保证一定会返回 thoughts。

#### mediaResolution `enum (MediaResolution)` <span style="color: #666; font-weight: 400">可选</span>
...
#### speechConfig `SpeechConfig` <span style="color: #666; font-weight: 400">可选</span>

语音生成配置；当 `responseModalities` 包含 `AUDIO` 时使用。

- `voiceConfig` `VoiceConfig` <span style="color: #666; font-weight: 400">可选</span>

  单一语音配置（与 `multiSpeakerVoiceConfig` 互斥）。
  - `prebuiltVoiceConfig` `PrebuiltVoiceConfig` <span style="color: #666; font-weight: 400">可选</span>

    预置语音配置。
    - `voiceName` `string` <span style="color: #666; font-weight: 400">可选</span>

      预置语音名称（如 `Kore`、`Puck`、`Charon` 等）。

  - `replicatedVoiceConfig` `ReplicatedVoiceConfig` <span style="color: #666; font-weight: 400">可选</span>
    ...
    - `voiceSample` `string(bytes)` <span style="color: #666; font-weight: 400">可选</span>

      自定义语音样本（base64 编码）。

- `multiSpeakerVoiceConfig` `MultiSpeakerVoiceConfig` <span style="color: #666; font-weight: 400">可选</span>
...
- `languageCode` `string` <span style="color: #666; font-weight: 400">可选</span>

  语音输出的语言代码（如 `en-US`）。

#### routingConfig `RoutingConfig` <span style="color: #666; font-weight: 400">可选</span>

路由配置（Vertex AI）：将请求路由到特定模型。`autoMode` 和 `manualMode` 互斥。

> **注意**：此字段已废弃（deprecated），Google 建议使用 `modelConfig` 替代。

- `autoMode` `AutoRoutingMode` <span style="color: #666; font-weight: 400">可选</span>

  自动路由：由预训练的路由模型和用户偏好共同决定路由。
  - `modelRoutingPreference` `enum (ModelRoutingPreference)` <span style="color: #666; font-weight: 400">可选</span>

    路由偏好。支持值：`BALANCED` 等。

- `manualMode` `ManualRoutingMode` <span style="color: #666; font-weight: 400">可选</span>

  手动路由：直接指定目标模型。
  - `modelName` `string` <span style="color: #666; font-weight: 400">可选</span>

    目标模型名称（如 `gemini-1.5-pro-001`）。

#### imageConfig `ImageConfig` <span style="color: #666; font-weight: 400">可选</span>
...
- `imageSize` `string` <span style="color: #666; font-weight: 400">可选</span>

  生成图像的尺寸。支持值：`1K`、`2K`、`4K`。默认 `1K`。

- `outputCompressionQuality` `number` <span style="color: #666; font-weight: 400">可选</span>

  生成图像的压缩质量（仅 `image/jpeg` 格式适用）。

- `outputMimeType` `string` <span style="color: #666; font-weight: 400">可选</span>

  生成图像的 MIME 类型。

#### enableAffectiveDialog `boolean` <span style="color: #666; font-weight: 400">可选</span>

是否启用情感对话：开启后模型会检测用户情绪并据此调整回复风格。

### systemInstruction `Content` <span style="color: #666; font-weight: 400">可选</span>
