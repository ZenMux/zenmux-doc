...
| 模型                            | 输出维度         | 最大输入 Token |
| ------------------------------- | ---------------- | -------------- |
| `openai/text-embedding-3-large` | 3072（可自定义） | 8191           |
| `openai/text-embedding-3-small` | 1536（可自定义） | 8191           |

::: tip 关于维度
`text-embedding-3-large` 默认输出 3072 维向量，`text-embedding-3-small` 默认输出 1536 维向量。你可以通过 `dimensions` 参数指定更低的维度（如 1024、512、256），在几乎不损失语义表达能力的前提下降低存储和检索成本。`text-embedding-3-large` 在语义精度上优于 `text-embedding-3-small`，适合对检索质量要求更高的场景。
:::
...
```ts [TypeScript]
const texts = ["如何重置密码？", "忘记密码怎么办？", "修改登录邮箱的方法"];
...
- 追求最佳精度：使用 `text-embedding-3-large`（默认 3072 维）
- 高精度且控制成本：使用 `text-embedding-3-large` 并将维度缩减至 1024 或 512
- 平衡精度和成本：使用 `text-embedding-3-small`（默认 1536 维）
