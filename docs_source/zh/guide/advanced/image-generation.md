...
# 图片生成-Google Gemini 协议

ZenMux 的生图协议目前有 **Google Gemini** 和 **OpenAI Image** 两种。

对于所有的生图模型，ZenMux 均支持通过 Google Gemini 协议调用；对于 OpenAI 系列的生图模型，则同时支持 OpenAI Image 协议和 Google Gemini 协议。

本指南主要介绍 Google Gemini 协议的用法。如果你希望使用 OpenAI Image 协议调用 OpenAI 系列图片模型，请参考 [OpenAI Image 协议图片生成](/zh/guide/advanced/openai-image-generation)。
...
ZenMux 的图片生成模型会持续更新。请前往 [ZenMux 模型列表](https://zenmux.ai/models?sort=newest&output_modalities=image) 查看当前所有支持 Google Gemini 协议的生图模型。

::: info OpenAI Image 协议模型
如果你只想查看支持 OpenAI Image 协议的 OpenAI 图片模型，请访问 [OpenAI 图片模型列表](https://zenmux.ai/models?author=openai&sort=newest&output_modalities=image)。
:::
...
| `size` | `config.http_options.extra_body.imageSize` | string | 图片尺寸(**透传参数**)。常见取值:`1024x1024`、`1536x1024`(横版)、`1024x1536`(竖版)、`auto`。底层模型支持的其他尺寸也可使用,详见 OpenAI 官方文档及下方透传说明。 | ✅ |
| `quality` | `config.http_options.extra_body.quality` | string | 图片质量(**透传参数**)。常见取值:`low` / `medium` / `high` / `auto`。底层模型支持的任意值都可传入,详见 OpenAI 官方文档。 | ✅ |
...
| `size` | `config.http_options.extra_body.imageSize` | string | 图片尺寸(**透传参数**)。常见取值:`1024x1024`、`1536x1024`、`1024x1536`、`auto`。底层模型支持的其他尺寸也可使用,详见 OpenAI 官方文档。 | ✅ |
| `quality` | `config.http_options.extra_body.quality` | string | 图片质量(**透传参数**)。常见取值:`low` / `medium` / `high` / `auto`。底层模型支持的任意值都可传入,详见 OpenAI 官方文档。 | ✅ |
...
`imageSize` 和 `quality` 属于**透传参数** —— ZenMux 会将其原样转发到底层 OpenAI 兼容的图片生成 API,不做额外校验和转换。上表中列出的取值是最常用的几种,完整的支持值与行为细节请参考 OpenAI 官方文档([图片生成](https://developers.openai.com/api/reference/resources/images/methods/generate)、[图片编辑](https://developers.openai.com/api/reference/resources/images/methods/edit)),你可以根据需要自行设置。

`imageSize`、`quality` 等 OpenAI 特有参数请通过 `httpOptions.extraBody` 透传,以确保在 Python 与 TypeScript SDK 中行为一致;`numberOfImages`、`outputMimeType`、`outputCompressionQuality` 等标准 Vertex AI 字段直接放在 `config` 顶层即可。
