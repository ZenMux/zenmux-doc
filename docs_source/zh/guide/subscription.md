...
::: warning 汇率说明
Flow/USD 汇率当前锚定在 **1 Flow ≈ $0.03283**（约 30 Flows = $1）。该汇率可能会根据市场状况和模型定价变化进行周期性调整。最新汇率将始终在此实时公布和展示。
:::

::: tip Insider 会员 Flow 价值
...
1. **全场景模型覆盖**
   ...
   | 模型类型        | 代表模型                                               |
   | --------------- | ------------------------------------------------------ |
   | **Coding 模型** | Claude Opus 4.7 / GPT-5.4 / Gemini-3.1-Pro-Preview ... |
   | **生图模型**    | NanoBananaPro / GPT-Image-1.5 ...（陆续上线）          |
   | **生文模型**    | GPT-5.4 / Qwen3.6-Plus / GLM 5.1 / ERNIE 5.0 ...       |

2. **全明星模型矩阵**

   一个订阅，同时调度全球顶尖模型（Gemini 2.5 Pro、GPT-5 系列、Claude Opus/Sonnet 4 系列、Grok 4 系列等）。最新顶尖模型第一时间使用，像拥有全网最强算力协同作战。
...
- `sapiens-ai/agnes-image-1.2` - Sapiens AI: Agnes Image 1.2（生图模型）
...
> 以下 Doubao 模型同时支持 `bytedance/*` 和 `volcengine/*` 两种 slug 前缀，功能完全相同。
...
订阅用户在每个计费周期（月）内拥有一定次数的 **5 小时窗口重置** 机会。当 5 小时配额即将用尽时，您可以手动重置窗口，立即恢复配额继续使用，无需等到自然滚动重置。
...
::: info 提示
各套餐的重置次数未来可能会根据运营策略调整，请以 [订阅管理页面](https://zenmux.ai/platform/subscription) 实际显示为准。
:::
...
除了通过 API Key 在开发工具中使用,你还可以在 **[ZenMux Studio Chat](https://zenmux.ai/chat)** 网页端直接使用订阅制配额进行对话。
...
升级时仅需补差价 **$40**；新周期重置后恢复到 Max 的正常月费 $100。

::: tip 计费周期一致

升级后原有的计费周期**不会重置**——下一次续费日期仍与升级前保持一致。这样能避免"升级即续费"导致的双重扣费。

:::

## 退款

如需申请退款,请参阅 [退款说明](./refund) 了解退款资格、计算方式和操作流程。
...
您可以在[订阅管理页面](https://zenmux.ai/platform/subscription)实时查看剩余配额和重置时间。
