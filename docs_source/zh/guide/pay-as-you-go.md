...
::: info 余额说明
所有额度以美元计价（$），1 额度 = 1 美元等值的 API 调用量。按量付费消耗顺序：**Token 包（如有）→ 奖励与补偿额度 → 充值额度**。完整的扣减优先级（含订阅额度）见 [Token 包文档](/zh/guide/token-pack)。
:::
...
### 我的 Token 包和这里的余额是什么关系？

**Token 包（Token Pack）** 是独立的定量 Token 权益，不体现在 Total Balance 里；按量消耗时它**优先于**余额扣减，用尽后自动切到余额。参见 [Token 包](/zh/guide/token-pack)。
