...
进入 **Identity & Models** 或模型相关配置区域，配置您希望在 Claude Desktop 中使用的模型。

Claude Desktop 的 Gateway Base URL 和模型列表是两类配置：

- **Gateway Base URL** 决定请求发往哪里，例如 `https://zenmux.ai/api/anthropic`
- **Model List / Model Discovery** 决定模型选择器里显示哪些模型

#### 方式 A：手动配置 Model List

这是最稳定的方式，适合您希望明确控制 Claude Desktop 中展示哪些模型。

在 Model List 中添加模型 slug。模型 slug 会作为实际请求中的 `model` 发送给 ZenMux。
...
#### 方式 B：使用 Model Discovery

如果开启 **Model Discovery**，Claude Desktop 会尝试从 Gateway 的模型列表接口自动拉取模型。对于 ZenMux Anthropic 兼容端点，对应接口为：

```text
https://zenmux.ai/api/anthropic/v1/models
```

这种方式配置更少，适合快速测试。但在某些 Claude Desktop 版本中，自动发现结果可能会被客户端筛选、别名解析或重新排序。如果您发现模型选择器中的模型数量少于 ZenMux `/v1/models` 返回值，建议改用手动 Model List。

#### 方式 C：通过 JSON 批量写入 inferenceModels

如果您需要一次性导入较多模型，可以点击配置界面的 **View as JSON**，批量写入 `inferenceModels`。
...
- `name` 是实际发送给 ZenMux 的模型 slug
- `labelOverride` 只影响 Claude Desktop 中的显示名称
- 关闭 `modelDiscoveryEnabled` 后，`inferenceModels` 将作为固定模型列表使用

::: tip 社区工具
如果您需要从 ZenMux `/v1/models` 自动生成 `inferenceModels`，可以参考社区项目 [Lovmux](https://github.com/lovstudio/lovmux) 或其他配置管理工具。
:::
