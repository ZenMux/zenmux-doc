...
- `features.multi_agent_v2.tool_namespace`: 为多智能体工具指定 `agents` 命名空间；使用下文列出的 GPT-5.6 模型时建议保留此配置
:::
...
### GPT-5.6 Luna、Terra 与 Sol 的多智能体配置

::: warning 重要提醒
使用最新的 `gpt-5.6-luna`、`gpt-5.6-terra` 或 `gpt-5.6-sol` 时，如果遇到以下错误：
...
请在 `~/.codex/config.toml` 中添加以下配置（上方默认示例已包含）：
